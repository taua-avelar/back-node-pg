...
const excluirAutor = async (req, res) => {
  ...
  try {
    ...
    const queryLivros = "select * from livros where autor_id = $1";
    const livrosAtrelados = await conexao.query(queryLivros, [id]);

    if (livrosAtrelados.rowCount !== 0)
      return res.status(400).json("Existem livros atrelados a esse autor");
